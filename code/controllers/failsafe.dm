				...
				if(Master.iteration == master_iteration)
					switch(defcon)
						if(4,5)
							--defcon
						if(3)
							to_chat(GLOB.admins, "<span class='boldannounce'>Notice: DEFCON [defcon_pretty()]. The Master Controller has not fired in the last [(5-defcon) * processing_interval] ticks.</span>")
							--defcon
						if(2)
						...
						if(1)
							...
							if(rtn > 0)
								defcon = 4
								master_iteration = 0
								to_chat(GLOB.admins, "<span class='boldannounce'>MC restarted successfully</span>")
							else if(rtn < 0)
