...
/decl/communication_channel/pray/do_communicate(mob/communicator, message, speech_method_type)
	...
	for(var/m in GLOB.player_list)
		...
		if(M.client.holder && M.client.get_preference_value(/datum/client_preference/staff/show_chat_prayers) == GLOB.PREF_SHOW)
			receive_communication(communicator, M, "\[<A HREF='?_src_=holder;adminspawncookie=\ref[communicator]'>SC</a>\] \[<A HREF='?_src_=holder;take_ic=\ref[src]'>TAKE</a>\][SPAN("notice", "\icon[cross] <b><font color=purple>PRAY: </font>[key_name(communicator, 1)]: </b>[message]")]")
