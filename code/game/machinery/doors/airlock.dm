...
/obj/machinery/door/airlock/attackby(obj/item/C, mob/user)
	...
	if(!brace && istype(C, /obj/item/airlock_brace))
		...
		if(!(length(A.req_access) || length(A.req_one_access)))
			var/answer = alert("\the [A]'s 'Access Not Set' light is flashing. Install it anyway?", "Access not set", "Yes", "No")
			if(answer == "No")
				return

		if(do_after(user, 50, src) && density && user.drop(A, src))
...
/obj/machinery/door/airlock/proc/lock(forced = 0)
	...
	audible_message("You hear a click from the bottom of the door.", hearing_distance = 1, runechat_message = "*click*")
	update_icon()
	return 1
