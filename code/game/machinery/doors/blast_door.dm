...
/obj/machinery/door/blast/attackby(obj/item/C as obj, mob/user as mob)
	...
	if((isCrowbar(C) && !istype(C, /obj/item/crowbar/emergency)) || (istype(C, /obj/item/material/twohanded/fireaxe) && C:wielded == 1))
		if(((stat & NOPOWER) || (stat & BROKEN)) && !( src.operating ))
			...
			to_chat(usr, SPAN("notice", "[src]'s motors resist your effort."))
		return
	if(istype(C, /obj/item/stack/material) && C.get_material_name() == MATERIAL_PLASTEEL)
		var/amt = Ceiling((maxhealth - health)/150)
		if(!amt)
			to_chat(usr, SPAN("notice", "\The [src] is already fully repaired."))
			return
		var/obj/item/stack/P = C
		if(P.amount < amt)
			to_chat(usr, SPAN("warning", "You don't have enough sheets to repair this! You need at least [amt] sheets."))
			return
		to_chat(usr, SPAN("notice", "You begin repairing [src]..."))
		if(do_after(usr, 30, src))
			if(P.use(amt))
				to_chat(usr, SPAN("notice", "You have repaired \the [src]"))
				src.repair()
			else
				to_chat(usr, SPAN("warning", "You don't have enough sheets to repair this! You need at least [amt] sheets."))
