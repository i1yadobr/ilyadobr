...
/obj/machinery/porta_turret/proc/isLocked(mob/user)
	if(ailock && issilicon(user))
		to_chat(user, SPAN("notice", "There seems to be a firewall preventing you from accessing this device."))
		return 1

	if(locked && !issilicon(user))
		to_chat(user, SPAN("notice", "Access denied."))
		return 1
...
/obj/machinery/porta_turret/CanUseTopic(mob/user)
	if(HasController())
		to_chat(user, SPAN("notice", "Turrets can only be controlled using the assigned turret controller."))
		return STATUS_CLOSE
	...
	if(!anchored)
		to_chat(usr, SPAN("notice", "\The [src] has to be secured first!"))
		return STATUS_CLOSE
...
/obj/machinery/porta_turret/attackby(obj/item/I, mob/user)
	if(stat & BROKEN)
		if(isCrowbar(I))
			...
			to_chat(user, SPAN("notice", "You begin prying the metal coverings off."))
			if(do_after(user, 20, src))
				if(prob(70))
					...
					to_chat(user, SPAN("notice", "You remove the turret but did not manage to salvage anything."))
				qdel(src) // qdel

	else if(isWrench(I))
		if(enabled || raised)
			to_chat(user, SPAN("warning", "You cannot unsecure an active turret!"))
			return
		if(wrenching)
			to_chat(user, SPAN("warning", "Someone is already [anchored ? "un" : ""]securing the turret!"))
			return
		if(!anchored && isinspace())
			to_chat(user, SPAN("warning", "Cannot secure turrets in space!"))
			return

		user.visible_message( \
				SPAN("warning", "[user] begins [anchored ? "un" : ""]securing the turret."), \
				SPAN("notice", "You begin [anchored ? "un" : ""]securing the turret.") \
			)
		...
		if(do_after(user, 50, src))
			...
			if(!anchored)
				...
				anchored = 1
				update_icon()
				to_chat(user, SPAN("notice", "You secure the exterior bolts on the turret."))
			else if(anchored)
				playsound(loc, 'sound/items/Ratchet.ogg', 100, 1)
				anchored = 0
				to_chat(user, SPAN("notice", "You unsecure the exterior bolts on the turret."))
				update_icon()
		wrenching = 0
...
/obj/machinery/porta_turret/emag_act(remaining_charges, mob/user)
	if(!emagged)
		...
		to_chat(user, SPAN("warning", "You short out [src]'s threat assessment circuits."))
	...
	switch(build_step)
		if(0)	//first step
			if(isWrench(I) && !anchored)
				playsound(loc, 'sound/items/Ratchet.ogg', 100, 1)
				to_chat(user, SPAN("notice", "You secure the external bolts."))
				anchored = 1
				build_step = 1
				return
				...
				to_chat(user, SPAN("notice", "You dismantle the turret construction."))
		...
		if(1)
			if(istype(I, /obj/item/stack/material) && I.get_material_name() == MATERIAL_STEEL)
				...
				if(M.use(2))
					to_chat(user, SPAN("notice", "You add some metal armor to the interior frame."))
					build_step = 2
					icon_state = "turret_frame2"
				else
					to_chat(user, SPAN("warning", "You need two sheets of metal to continue construction."))
				return

			else if(istype(I, /obj/item/wrench))
				playsound(loc, 'sound/items/Ratchet.ogg', 75, 1)
				to_chat(user, SPAN("notice", "You unfasten the external bolts."))
				anchored = 0
				build_step = 0
				return
		...
		if(2)
			if(istype(I, /obj/item/wrench))
				playsound(loc, 'sound/items/Ratchet.ogg', 100, 1)
				to_chat(user, SPAN("notice", "You bolt the metal armor into place."))
				build_step = 3
				return
				...
				if(WT.get_fuel() < 5) //uses up 5 fuel.
					to_chat(user, SPAN("notice", "You need more fuel to complete this task."))
					return
		...
		if(3)
			if(istype(I, /obj/item/gun/energy)) //the gun installation part
				...
				if(!user.drop(I))
					to_chat(user, SPAN("notice", "\the [I] is stuck to your hand, you cannot put it in \the [src]"))
					return
				installation = I.type //installation becomes I.type
				gun_charge = E.power_supply.charge //the gun's charge is stored in gun_charge
				to_chat(user, SPAN("notice", "You add [I] to the turret."))
				...
				playsound(loc, 'sound/items/Ratchet.ogg', 100, 1)
				to_chat(user, SPAN("notice", "You remove the turret's metal armor bolts."))
				build_step = 2
				return
		...
		if(4)
			if(isprox(I))
				...
				if(!user.drop(I))
					to_chat(user, SPAN("notice", "\the [I] is stuck to your hand, you cannot put it in \the [src]"))
					return
				to_chat(user, SPAN("notice", "You add the prox sensor to the turret."))
				qdel(I)
				return
		...
		if(5)
			if(isScrewdriver(I))
				...
				build_step = 6
				to_chat(user, SPAN("notice", "You close the internal access hatch."))
				return
		...
		if(6)
			if(istype(I, /obj/item/stack/material) && I.get_material_name() == MATERIAL_STEEL)
				var/obj/item/stack/M = I
				if(M.use(2))
					to_chat(user, SPAN("notice", "You add some metal armor to the exterior frame."))
					build_step = 7
				else
					to_chat(user, SPAN("warning", "You need two sheets of metal to continue construction."))
				return
				...
				playsound(loc, 'sound/items/Screwdriver.ogg', 100, 1)
				build_step = 5
				to_chat(user, SPAN("notice", "You open the internal access hatch."))
				return
			...
			if(isWelder(I))
				...
				if(do_after(user, 30, src))
					if(!src || !WT.remove_fuel(5, user))
						return
					build_step = 8
					to_chat(user, SPAN("notice", "You weld the turret's armor down."))
				...
				playsound(loc, 'sound/items/Crowbar.ogg', 75, 1)
				to_chat(user, SPAN("notice", "You pry off the turret's exterior armor."))
				new /obj/item/stack/material/steel(loc, 2)
				build_step = 6
				return
