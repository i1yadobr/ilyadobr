...
/mob/living/silicon/ai/proc/ai_store_location(loc as text)
	...
	loc = sanitize(loc)
	if(!loc)
		to_chat(src, SPAN("warning", "Must supply a location name"))
		return

	if(stored_locations.len >= max_locations)
		to_chat(src, SPAN("warning", "Cannot store additional locations. Remove one first"))
		return

	if(loc in stored_locations)
		to_chat(src, SPAN("warning", "There is already a stored location by this name"))
		return

	var/L = src.eyeobj.getLoc()
	if (InvalidPlayerTurf(get_turf(L)))
		to_chat(src, SPAN("warning", "Unable to store this location"))
		return
...
/mob/living/silicon/ai/proc/ai_goto_location(loc in sorted_stored_locations())
	...
	if (!(loc in stored_locations))
		to_chat(src, SPAN("warning", "Location [loc] not found"))
		return
...
/mob/living/silicon/robot/tracking_cancelled()
	...
	if(!tracking_entities && has_zeroth_law())
		to_chat(src, SPAN("notice", "Internal camera is no longer being accessed."))
