...
/datum/malf_hardware/apu_gen/get_examine_desc()
	...
	if(owner.hardware_integrity() < 50)
		msg += SPAN("warning", " It seems to be too damaged to function properly.")
...
/datum/malf_hardware/core_bomb/get_examine_desc()
	return SPAN("warning", "It seems to have grey blocks of unknown substance and some circuitry connected to it's core. [owner.bombing_core ? "A red light is blinking on the circuit." : ""]")
