...
/datum/malf_research_ability/passive/relay_suppression/research_finished(mob/living/silicon/ai/user)
	...
	if(!user)
		return
	to_chat(user, SPAN("notice", "You have suppressed the IDS system of nearby quantum relay. Your hacks will no longer be prevented or detected."))
...
/datum/malf_research_ability/passive/relay_override/research_finished(mob/living/silicon/ai/user)
	...
	if(!user)
		return
	to_chat(user, SPAN("notice", "You have completely overtaken a nearby quantum relay. No remote communications will work."))
