...
/mob/observer/ghost/proc/ghost_ability_check()
	...
	if(T.holy)
		to_chat(src, SPAN("notice", "You may not use your abilities on the blessed ground."))
		return 0
	if(ghost_magic_cd > world.time)
		to_chat(src, SPAN("notice", "You need [round((ghost_magic_cd - world.time) / 10)] more seconds before you can use your abilities."))
		return 0
	return 1
...
/mob/observer/ghost/proc/bloody_doodle_proc(bloodless = 0)
	...
	if(!bloodless)
		...
		if(!choices.len)
			to_chat(src, SPAN("warning", "There is no blood to use nearby."))
			return
...
/mob/observer/ghost/proc/move_item()
	...
	if(!choices.len)
		to_chat(src, SPAN("warning", "There are no suitable items nearby."))
		return
...
/mob/observer/ghost/proc/whisper_proc(anyone = 0)
	...
	if(message)
		if(iscultist(choice) || anyone)
			to_chat(choice, SPAN("notice", "You hear a faint whisper... It says... \"[message]\""))
			log_and_message_admins("used ghost magic to say '[message]' to \the [choice] and was heard - [x]-[y]-[z]")
		else
			to_chat(choice, SPAN("notice", "You hear a faint whisper, but you can't make out the words."))
...
/mob/observer/ghost/proc/bite_someone()
	...
	to_chat(choice, SPAN("danger", "Something invisible [method] you!"))
	choice.apply_effect(5, PAIN, 0)
	to_chat(src, SPAN("notice", "You [method] \the [choice]."))
...
/mob/observer/ghost/proc/chill_someone()
	...
	to_chat(choice, SPAN("danger", "You feel as if something cold passed through you!"))
	...
	to_chat(src, SPAN("notice", "You pass through \the [choice], giving them a sudden chill."))
...
/mob/observer/ghost/proc/toggle_visiblity()
	...
	if(invisibility == 0)
		ghost_magic_cd = world.time + 60 SECONDS
		to_chat(src, SPAN("info", "You are now invisible."))
		visible_message(SPAN("emote", "It fades from sight..."))
		set_invisibility(INVISIBILITY_OBSERVER)
		mouse_opacity = 1
	else
		ghost_magic_cd = world.time + 60 SECONDS
		to_chat(src, SPAN("info", "You are now visible."))
		set_invisibility(0)
