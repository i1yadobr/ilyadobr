	...
	for(var/mob/living/carbon/M in oviewers(8, src))
		if(M.stat == CONSCIOUS)
			if(M.status_flags & GODMODE)
				continue
			if(!iscultist(M))
				to_chat(M, SPAN("danger", " You feel your sanity crumble away in an instant as you gaze upon [src.name]..."))
...
/obj/singularity/narsie/proc/acquire(const/mob/food)
	...
	to_chat(target, SPAN("notice", "<b>[capname] HAS LOST INTEREST IN YOU.</b>"))
	target = food

	if (ishuman(target))
		to_chat(target, SPAN("danger", "[capname] HUNGERS FOR YOUR SOUL."))
	else
		to_chat(target, SPAN("danger", "[capname] HAS CHOSEN YOU TO LEAD HIM TO HIS NEXT MEAL."))
