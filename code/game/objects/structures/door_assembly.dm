...
/obj/structure/door_assembly/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(isWelder(W) && ( (istext(glass)) || (glass == 1) || (!anchored) ))
		...
		if (WT.remove_fuel(0, user))
			...
			if(istext(glass))
				...
				if(do_after(user, 40,src))
					...
					to_chat(user, SPAN("notice", "You welded the [glass] plating off!"))
				...
				if(do_after(user, 40,src))
					if(!src || !WT.isOn()) return
					to_chat(user, SPAN("notice", "You welded the glass panel out!"))
				...
				if(do_after(user, 40,src))
					if(!src || !WT.isOn()) return
					to_chat(user, SPAN("notice", "You dissasembled the airlock assembly!"))
					new /obj/item/stack/material/steel(src.loc, 4)
					qdel (src)
		else
			to_chat(user, SPAN("notice", "You need more welding fuel."))
			return
		...
		if(do_after(user, 40,src))
			if(!src) return
			to_chat(user, SPAN("notice", "You finish the airlock!"))
			var/path
			if(istext(glass))
