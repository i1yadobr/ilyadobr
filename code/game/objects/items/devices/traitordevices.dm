...
/obj/item/device/batterer/attack_self(mob/living/carbon/user as mob, flag = 0, emp = 0)
	...
	if(times_used >= max_uses)
		to_chat(user, SPAN("warning", "The mind batterer has been burnt out!"))
		return
	...
	for(var/mob/living/carbon/human/M in orange(10, user))
		...
		spawn()
			if(prob(50))
				...
				if(prob(25))
					M.Stun(rand(5,10))
				to_chat(M, SPAN("danger", "You feel a tremendous, paralyzing wave flood your mind."))
			else
				to_chat(M, SPAN("danger", "You feel a sudden, electric jolt travel through your head."))
