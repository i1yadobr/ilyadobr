...
/datum/antagonist/wizard
	...
	flags = ANTAG_OVERRIDE_JOB | ANTAG_OVERRIDE_MOB | ANTAG_CLEAR_EQUIPMENT | ANTAG_CHOOSE_NAME | ANTAG_VOTABLE | ANTAG_SET_APPEARANCE | ANTAG_DISPLAY_IN_CHARSETUP
...
/mob/living/carbon/human/wearing_wiz_garb()
	if(!is_wiz_garb(src.wear_suit) && (!src.species.hud || (slot_wear_suit in src.species.hud.equip_slots)))
		to_chat(src, SPAN("warning", "I don't feel strong enough without my robe."))
		return 0
	if(!is_wiz_garb(src.shoes) && (!species.hud || (slot_shoes in src.species.hud.equip_slots)))
		to_chat(src, SPAN("warning", "I don't feel strong enough without my sandals."))
		return 0
	if(!is_wiz_garb(src.head) && (!species.hud || (slot_head in src.species.hud.equip_slots)))
		to_chat(src, SPAN("warning", "I don't feel strong enough without my hat."))
		return 0
	return 1
