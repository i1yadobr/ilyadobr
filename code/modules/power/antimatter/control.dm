...
/obj/machinery/power/am_control_unit/attackby(obj/item/W, mob/user)
	...
	if(isWrench(W))
		if(!anchored)
			...
			to_chat(user, SPAN("warning", "Once bolted and linked to a shielding unit it the [src.name] is unable to be moved!"))
		return

	if(istype(W, /obj/item/am_containment))
		if(fueljar)
			to_chat(user, SPAN("warning", "There is already a [fueljar] inside!"))
			return
		fueljar = W
		user.drop(W, src)
		user.update_icons()
