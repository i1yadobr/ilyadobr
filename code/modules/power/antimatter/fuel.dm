...
/obj/item/fuel/proc/injest(mob/M as mob)
	switch(content)
		if("Anti-Hydrogen")
			M.gib()
		if("Hydrogen")
			to_chat(M, SPAN("notice", "You feel very light, as if you might just float away..."))
	qdel(src)
	return
...
/obj/item/fuel/attack(mob/M as mob, mob/user as mob)
	if (user != M)
		...
		for(var/mob/O in viewers(M, null))
			O.show_message(SPAN("warning", "[M] ate the [content ? content : "empty canister"]!"), VISIBLE_MESSAGE)
		src.injest(M)
