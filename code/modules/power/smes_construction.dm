...
/obj/machinery/power/smes/buildable/attack_ai()
	if(RCon)
		...
		to_chat(usr, SPAN("warning", "Connection error: Destination Unreachable."))
...
/obj/machinery/power/smes/buildable/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (failing)
		to_chat(user, SPAN("warning", "The [src]'s screen is flashing with alerts. It seems to be overloaded! Touching it now is probably not a good idea."))
		return
	...
	if (..())
		...
		if((charge > (capacity/100)) && safeties_enabled)
			to_chat(user, SPAN("warning", "Safety circuit of [src] is preventing modifications while it's charged!"))
			return

		if (output_attempt || input_attempt)
			to_chat(user, SPAN("warning", "Turn off the [src] first!"))
			return
		...
		if(isCrowbar(W))
			if (terminals.len)
				to_chat(user, SPAN("warning", "You have to disassemble the terminal first!"))
				return

			playsound(src, 'sound/items/Crowbar.ogg', 50, 1)
			to_chat(user, SPAN("warning", "You begin to disassemble the [src]!"))
			if (do_after(usr, 50 * cur_coils, src)) // More coils = takes longer to disassemble. It's complex so largest one with 6 coils will take 30s
				...
				to_chat(usr, SPAN("warning", "You have disassembled the SMES cell!"))
		...
		// Superconducting Magnetic Coil - Upgrade the SMES
			...
			if(cur_coils < max_coils)
				...
				to_chat(usr, SPAN("warning", "You can't insert more coils to this SMES unit!"))
