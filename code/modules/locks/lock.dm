...
/datum/lock/proc/unlock(key = "", mob/user)
	if(status ^ LOCK_LOCKED)
		to_chat(user, SPAN("warning", "Its already unlocked!"))
		return 2
...
/datum/lock/proc/lock(key = "", mob/user)
	if(status & LOCK_LOCKED)
		to_chat(user, SPAN("warning", "Its already locked!"))
		return 2
	key = get_key_data(key, user)
	if(cmptext(lock_data,key) && (status ^ LOCK_BROKEN))
		status |= LOCK_LOCKED
...
/datum/lock/proc/pick_lock(obj/item/I, mob/user)
	...
	if(prob(20*(unlock_power/getComplexity())))
		to_chat(user, SPAN("notice", "You pick open \the [holder]'s lock!"))
		unlock(lock_data)
		return 1
	else if(prob(5 * unlock_power))
		to_chat(user, SPAN("warning", "You accidently break \the [holder]'s lock with your [I]!"))
		status |= LOCK_BROKEN
	else
		to_chat(user, SPAN("warning", "You fail to pick open \the [holder]."))
	return 0
