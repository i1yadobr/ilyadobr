...
/obj/item/integrated_circuit/memory/constant/attack_self(mob/user)
	...
	switch(type_to_use)
		...
		if("ref")
			accepting_refs = TRUE
			to_chat(user, SPAN("notice", "You turn \the [src]'s ref scanner on.  Slide it across an object for a ref of that object to save it in memory."))
		if("null")
			O.data = null
...
/obj/item/integrated_circuit/memory/constant/afterattack(atom/target, mob/living/user, proximity)
	...
	if(accepting_refs && proximity)
		...
		to_chat(user, SPAN("notice", "You set \the [src]'s memory to a reference to [O.display_data(O.data)].  The ref scanner is now off."))
		accepting_refs = FALSE
