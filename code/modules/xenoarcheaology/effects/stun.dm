...
/datum/artifact_effect/stun/DoEffectTouch(mob/toucher)
	if(toucher && iscarbon(toucher))
		...
		if(prob(susceptibility * 100))
			to_chat(C, SPAN("warning", "A powerful force overwhelms your consciousness."))
	...
	if(holder)
		...
		for (var/mob/living/carbon/C in range(src.effectrange,T))
			...
			if(prob(10 * susceptibility))
				to_chat(C, SPAN("warning", "Your body goes numb for a moment."))
				C.Weaken(2)
				C.stuttering += 2
				if(prob(10))
					C.Stun(1)
			else if(prob(10))
				to_chat(C, SPAN("warning", "You feel numb."))
	...
	if(holder)
		...
		for (var/mob/living/carbon/C in range(src.effectrange,T))
			...
			if(prob(100 * susceptibility))
				to_chat(C, SPAN("warning", "A wave of energy overwhelms your senses!"))
