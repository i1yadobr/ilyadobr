	...
	if(isWelder(I))
		...
		if(W.isOn())
			if(W.get_fuel() >= 2)
				...
				if(inside)
					inside.forceMove(get_turf(src))
					visible_message(SPAN("info", "\The [src] burns away revealing \the [inside]."))
				else
					visible_message(SPAN("info", "\The [src] burns away into nothing."))
				qdel(src)
				W.remove_fuel(2)
			else
				visible_message(SPAN("info", "A few sparks fly off \the [src], but nothing else happens."))
				W.remove_fuel(1)
			return
