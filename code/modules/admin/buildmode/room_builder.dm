...
/datum/build_mode/room_builder/Help()
	...
	to_chat(user, SPAN("notice", "Left Click on Turf               = Select as point A"))
	to_chat(user, SPAN("notice", "Right Click on Turf              = Select as point B"))
	to_chat(user, SPAN("notice", "As soon as both points have been selected, the room is created."))
	to_chat(user, "")
	to_chat(user, SPAN("notice", "Right Click on Build Mode Button = Change floor/wall type"))
	to_chat(user, SPAN("notice", "***********************************************************"))
...
/datum/build_mode/room_builder/Configurate()
	...
	switch(choice)
		if("Floor")
			...
			to_chat(user, SPAN("notice", "Floor type set to [floor_type]."))
		if("Wall")
			wall_type = select_subpath(wall_type) || wall_type
			to_chat(user, SPAN("notice", "Wall type set to [wall_type]."))

/datum/build_mode/room_builder/OnClick(atom/A, list/parameters)
	if(parameters["left"])
		coordinate_A = get_turf(A)
		to_chat(user, SPAN("notice", "Defined [coordinate_A] ([coordinate_A.type]) as point A."))
	if(parameters["right"])
		coordinate_B = get_turf(A)
		to_chat(user, SPAN("notice", "Defined [coordinate_B] ([coordinate_B.type]) as point B."))

	if(coordinate_A && coordinate_B)
		to_chat(user, SPAN("notice", "Room coordinates set. Building room."))
