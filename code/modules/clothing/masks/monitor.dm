...
/obj/item/clothing/mask/monitor/equipped()
	...
	if(istype(H) && H.wear_mask == src)
		canremove = 0
		to_chat(H, SPAN("notice", "\The [src] connects to your display output."))
...
/obj/item/clothing/mask/monitor/mob_can_equip(mob/living/carbon/human/user, slot)
	...
	if(istype(user))
		...
		if(istype(E) && BP_IS_ROBOTIC(E))
			return 1
		to_chat(user, SPAN("warning", "You must have a robotic head to install this upgrade."))
	return 0
...
/obj/item/clothing/mask/monitor/verb/set_monitor_state()
	...
	if(!istype(H) || H != usr)
		return
	if(H.wear_mask != src)
		to_chat(usr, SPAN("warning", "You have not installed \the [src] yet."))
		return
