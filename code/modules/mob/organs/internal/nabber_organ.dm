...
/obj/item/organ/internal/eyes/nabber/verb/shield_eyes()
	...
	if(eyes_shielded)
		to_chat(owner, SPAN("notice", "Nearly opaque lenses slide down to shield your eyes."))
		innate_flash_protection = FLASH_PROTECTION_MAJOR
		owner.eye_blind = 20
		owner.update_icons()
	else
		to_chat(owner, SPAN("notice", "Your protective lenses retract out of the way."))
		innate_flash_protection = FLASH_PROTECTION_VULNERABLE
...
/obj/item/organ/internal/brain/nabber/think()
	...
	switch(blood_volume)
		if(BLOOD_VOLUME_OKAY to BLOOD_VOLUME_SAFE)
			...
			if(prob(1))
				to_chat(owner, SPAN("warning", "You're finding it difficult to move."))
		if(BLOOD_VOLUME_BAD to BLOOD_VOLUME_OKAY)
			lowblood_tally = 3 * lowblood_mult
			if(prob(1))
				to_chat(owner, SPAN("warning", "Moving has become very difficult."))
		if(BLOOD_VOLUME_SURVIVE to BLOOD_VOLUME_BAD)
			lowblood_tally = 5 * lowblood_mult
			if(prob(15))
				to_chat(owner, SPAN("warning", "You're almost unable to move!"))
		if(-(INFINITY) to BLOOD_VOLUME_SURVIVE)
			lowblood_tally = 6 * lowblood_mult
			if(prob(10))
				to_chat(owner, SPAN("warning", "Your body is barely functioning and is starting to shut down."))
				owner.Paralyse(1)
