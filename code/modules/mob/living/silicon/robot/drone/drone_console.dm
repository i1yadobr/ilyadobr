...
/obj/machinery/computer/drone_control/Topic(href, href_list)
	...
	if (href_list["setarea"])
		...
		to_chat(usr, SPAN("notice", "You set the area selector to [drone_call_area]."))

	else if (href_list["ping"])

		to_chat(usr, SPAN("notice", "You issue a maintenance request for all active drones, highlighting [drone_call_area]."))
		for(var/mob/living/silicon/robot/drone/D in world)
		...
		if(D.stat != 2)
			to_chat(usr, SPAN("danger", "You issue a law synchronization directive for the drone."))
			D.law_resync()
		...
		if(D.stat != 2)
			to_chat(usr, SPAN("danger", "You issue a kill command for the unfortunate drone."))
		...
		for(var/obj/machinery/drone_fabricator/fab in oview(3,src))
			...
			dronefab = fab
			to_chat(usr, SPAN("notice", "Drone fabricator located."))
			return

		to_chat(usr, SPAN("danger", "Unable to locate drone fabricator."))
		...
		to_chat(usr, SPAN("notice", "You [dronefab.produce_drones ? "enable" : "disable"] drone production in the nearby fabricator."))
