...
docker rm -f oldonyx > /dev/null 2>&1
docker image rm oldonyx-server > /dev/null 2>&1

echo "Building docker image!"

docker build -t oldonyx-server . || (echo "Failed to build the image" && exit)

echo
echo "Image built, starting..."

docker run -d -p 14076:14076 -e VOLUMES_FILESYSTEM_WORKAROUND=false -v $(pwd)/config:/home/server/oldonyx/config -v $(pwd)/data:/home/server/oldonyx/data --name oldonyx oldonyx-server || (echo "Failed to start the container" && exit)
